// components/mobileWelding/MobileWeldingSection.tsx
import { useRef, useEffect, useState } from 'react';
import gsap from 'gsap';
import { ScrollTrigger } from 'gsap/ScrollTrigger';
import { Zap, Move3d, Eye, Cpu, Shield, Camera, ChevronRight, ChevronLeft } from 'lucide-react';
import { Language } from '../../hooks/useLanguage';
import { ProductModal } from '../ProductModal';

interface MobileWeldingSectionProps {
  language: Language;
}

export const MobileWeldingSection = ({ language }: MobileWeldingSectionProps) => {
  ...
  const products = [
    {
      ...
      specs: [
        { label: language === 'en' ? 'Total Power' : language === 'es' ? 'Potencia Total' : 'Potência Total', value: '37KVA', icon: <Zap className="w-5 h-5" /> },
        ...
        { label: language === 'en' ? 'Repeatability' : language === 'es' ? 'Repetibilidad' : 'Repetibilidade', value: '±0.05mm', icon: <Eye className="w-5 h-5" /> },
        { label: language === 'en' ? 'Power Supply' : language === 'es' ? 'Fuente de Alimentación' : 'Fonte de Alimentação', value: '3x380V±10%', icon: <Cpu className="w-5 h-5" /> },
        { label: language === 'en' ? 'Battery' : language === 'es' ? 'Batería' : 'Bateria', value: '48V / 73ah', icon: <Zap className="w-5 h-5" /> },
        { label: language === 'en' ? 'Weight' : language === 'es' ? 'Peso' : 'Peso', value: '2200kg', icon: <Shield className="w-5 h-5" /> },
      ],
      features: [
        {
    ...
    },
    {
      ...
      specs: [
        ...
        { label: language === 'en' ? 'Robot Arm Range' : language === 'es' ? 'Alcance del Brazo' : 'Alcance do Braço', value: '2010mm', icon: <Move3d className="w-5 h-5" /> },
        { label: language === 'en' ? 'Repeatability' : language === 'es' ? 'Repetibilidad' : 'Repetibilidade', value: '±0.05mm', icon: <Eye className="w-5 h-5" /> },
        { label: language === 'en' ? 'Scanning Frame Rate' : language === 'es' ? 'Tasa de Escaneo' : 'Taxa de Varredura', value: '2000 FPS', icon: <Camera className="w-5 h-5" /> },
        { label: language === 'en' ? 'Travel Speed' : language === 'es' ? 'Velocidad de Desplazamiento' : 'Velocidade de Deslocamento', value: '2.5km/h', icon: <Move3d className="w-5 h-5" /> },
        { label: language === 'en' ? 'Weight' : language === 'es' ? 'Peso' : 'Peso', value: '1650kg', icon: <Shield className="w-5 h-5" /> },
      ],
      features: [
        {
    ...
    const intervals = products.map((_, index) => {
      return setInterval(() => {
        setCurrentSlides(prev => {
          ...
          newSlides[index] = (newSlides[index] + 1) % products[index].images.length;
          return newSlides;
        });
      }, 4000);
    });
  ...
  return (
    <div ref={sectionRef} id="products" className="bg-white pt-20">
